...
sql_1 <- paste0("
SELECT *
FROM `mech-vent-data.mv.mechanical-vent`
WHERE subject_id IN (", paste0(10000000:10050000, collapse=","), ")")

sql_2 <- "SELECT * FROM `mech-vent-data.mv.mechanical-vent` LIMIT 1000"
...
sql_7 <- "SELECT subject_id, hr, death_outcome, discharge_outcome, invasive, noninvasive,
highflow, fio2, heart_rate, sbp, dbp, mbp, resp_rate, temperature, spo2, sepsis3, sofa_24hours,
gender, anchor_age, insurance, race, los, elixhauser_vanwalraven FROM `mech-vent-data.mv.mechanical-vent`
ORDER BY subject_id, stay_id, hr"

sql_8 <- "SELECT * FROM `mech-vent-data.mv.mechanical-vent`
ORDER BY subject_id, stay_id, hr"


# Run the query and store the data in a tibble
query <- bq_project_query("mech-vent-data", sql_8)  ##yes login initiated here
